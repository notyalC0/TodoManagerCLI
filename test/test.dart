...
import '../bin/main.dart'; // Certifique-se de que o caminho aponta para o seu arquivo de código
...
void main() {
  group('Testes de TodoManager', () {
    ...
    test('Deve calcular as estatísticas corretamente usando IDs manuais', () {
      ...
      manager.carregar([
        {'id': 101, 'titulo': 'T1', 'descricao': '', 'prioridade': 3, 'concluida': false},
        {'id': 102, 'titulo': 'T2', 'descricao': '', 'prioridade': 3, 'concluida': false},
        {'id': 103, 'titulo': 'T3', 'descricao': '', 'prioridade': 3, 'concluida': false},
      ]);
      ...
      expect(manager.concluidas, equals(1), reason: 'Apenas uma tarefa deveria estar concluída');
      expect(manager.pendentes, equals(2));
      expect(manager.pct, closeTo(0.33, 0.01));
    });

    test('A lista deve ser ordenada por: Status > Prioridade > ID (Mais recente)', () {
      manager.carregar([
        {'id': 1, 'titulo': 'Baixa Antiga', 'prioridade': 3, 'concluida': false},
        {'id': 2, 'titulo': 'Baixa Nova', 'prioridade': 3, 'concluida': false},
        {'id': 3, 'titulo': 'Alta Urgência', 'prioridade': 1, 'concluida': false},
        {'id': 4, 'titulo': 'Já Concluída', 'prioridade': 1, 'concluida': true},
      ]);

      final lista = manager.tarefas;
    ...
    test('Deve editar uma tarefa mantendo a integridade do ID', () {
      manager.carregar([
        {'id': 500, 'titulo': 'Original', 'descricao': 'Desc', 'prioridade': 2, 'concluida': false}
      ]);

      manager.editar(500, 'Editado', 'Nova Desc', 1);

      final tarefa = manager.tarefas.first;
    ...
    test('Deve remover uma tarefa da lista pelo ID único', () {
      manager.carregar([
        {'id': 999, 'titulo': 'Deletar', 'descricao': '', 'prioridade': 3, 'concluida': false},
        {'id': 888, 'titulo': 'Ficar', 'descricao': '', 'prioridade': 3, 'concluida': false},
      ]);

      manager.excluir(999);

      expect(manager.total, equals(1));
      expect(manager.tarefas.first['id'], equals(888));
      expect(manager.tarefas.any((t) => t['id'] == 999), isFalse);
    });

    test('Deve alternar status de concluído para pendente corretamente', () {
      manager.carregar([
        {'id': 7, 'titulo': 'Teste', 'descricao': '', 'prioridade': 3, 'concluida': true}
      ]);
